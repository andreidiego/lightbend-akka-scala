...
object Man extends AutoPlugin {
  ...
  def command: Command = Command("man")(_ => optArg) { (state, arg) =>
    arg match {
      case Some(a) if a == "e" =>
  ...
  val codeRx: Regex = """(`)([^`]+)(`)""".r
  val fenceStartRx: Regex = """^```(bash|scala|java)$""".r
  ...
  def printOut(path: String) {
    ...
    IO.readLines(new sbt.File(path)) foreach {
      ...
      case ln if !inCodeFence && ln.matches(numberRx.toString() + ".*") =>
        val lne = numberRx replaceAllIn(ln, _ match {
          case numberRx(n, s) => f"$ConRed$n$s$ConReset"
        })
